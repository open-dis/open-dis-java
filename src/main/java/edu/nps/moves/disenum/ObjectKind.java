 ...
 * Use as ObjectKind.lookup[aVal] to get the enumeration that corresponds to a value.<p>
 * In non-realtime environments, the prefered method is getEnumerationForValue().
 */
static public ObjectKind lookup[] = new ObjectKind[8];

static private HashMap<Integer, ObjectKind>enumerations = new HashMap<Integer, ObjectKind>();

/* initialize the array and hash table at class load time */
static 
{
    for(ObjectKind anEnum:ObjectKind.values())
    {
...
static public String getDescriptionForValue(int aVal)
{
  String desc;
    ...
    ObjectKind val = enumerations.get(new Integer(aVal));
      if(val == null)
...
*/
static public ObjectKind getEnumerationForValue(int aVal) throws EnumNotFoundException
{
    ObjectKind val;
      val = enumerations.get(new Integer(aVal));
      if(val == null)
         throw new EnumNotFoundException("no enumeration found for value " + aVal + " of enumeration ObjectKind");
      return val;
}
