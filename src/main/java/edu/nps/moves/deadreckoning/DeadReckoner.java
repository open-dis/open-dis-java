...
public class DeadReckoner {
    ...
    public static void perform_DRv7(edu.nps.moves.dis7.EntityStatePdu pESPDU, double deltaTime) {
        ...
        // Process articulation parameters
        for (VariableParameter thisRateGeneric : pESPDU.getVariableParameters()) { // update articulation parameter values
            if (thisRateGeneric instanceof ArticulatedParts) {
                ...
                // Search for the corresponding art. part value parameter
                for (VariableParameter thisValGeneric : pESPDU.getVariableParameters()) {
                    if (thisValGeneric instanceof ArticulatedParts) {
                        ...
                        // found part's value
                        // update values based on type
                        switch ((int) (thisVal.getParameterType() % 32)) {
                            case 1:   // position
                            case 3:  // extension
                                thisVal.setParameterValue((float) (thisVal.getParameterValue() +
                                        thisRate.getParameterValue() * deltaTime));
                                if (thisVal.getParameterValue() > 1.0) {  // fully extended
                                    thisVal.setParameterValue(1.0f);
                                } else if (thisVal.getParameterValue() < 0.0) {  // fully retracted
                                    thisVal.setParameterValue(0.0f);
                                }
                                break;
                            case 5:  // x
                            case 7:  // y
                            ...
                            case 15:  // rotation
                                thisVal.setParameterValue((float) (thisVal.getParameterValue() +
                                        thisRate.getParameterValue() * deltaTime));
                                if (thisVal.getParameterValue() > Math.PI) {  // wrapping over 180 degrees
                                    thisVal.setParameterValue((float) (thisVal.getParameterValue() - 2.0 * Math.PI));
                                } else if (thisVal.getParameterValue() < -Math.PI) {  // wrapping under 180 degrees
                                    thisVal.setParameterValue((float) (thisVal.getParameterValue() + 2.0 * Math.PI));
                                }
                                break;
